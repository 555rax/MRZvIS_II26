...
def accuracy(model, Xn, Y, thr=0.5):
    correct = sum(int((model.predict(x) >= thr) == int(y[0])) for x, y in zip(Xn, Y))
...
def run_inference(model, A, B, c0, c1, name="Модель"):
    ...
    print(f"[{name}] A={A}, B={B} -> ŷ={y_hat:.3f} -> ближе к классу {nearest}")
    return y_hat, nearest


if __name__ == "__main__":
    mlp = MLP221(); mlp_epochs, mlp_Es = mlp.fit(Xn, Y)
    slp = SingleLayerPerceptron(); slp_epochs, slp_Es = slp.fit(Xn, Y)

    print("MLP 2-2-1:", mlp_epochs, "эп.,", round(mlp_Es,5), "точн.", accuracy(mlp, Xn, Y))
    print("SLP:", slp_epochs, "эп.,", round(slp_Es,5), "точн.", accuracy(slp, Xn, Y))

    while True:
        raw = input("Введите A B (пусто — выход): ").strip()
        if not raw:
            break
